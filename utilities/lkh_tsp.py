from typing import List, Optional, TextIO, Tuple, Any
...
def _solve_tsp_brute_force(
    ...
    x, fx = solve_tsp_brute_force_exact(distance_matrix)
    x = x or []
...
def solve_tsp_lin_kernighan(
    ...
    time_limit
        Maximum time in seconds to run the algorithm. If exceeded, returns
        the best solution found so far. If `None`, no time limit is imposed.
    ...
    while improvement:
        ...
        if time_limit is not None:
            ...
            if elapsed_time > time_limit:
                msg = f"Time limit exceeded ({time_limit}s). Returning best solution found."
                _print_message(msg, verbose, log_file_handler)
                break
...
def setup_initial_solution(
    ...
    fx0 = _compute_permutation_distance(distance_matrix, x0)
    return x0, fx0
...
#### exact ####
def solve_tsp_brute_force_exact(
    distance_matrix: np.ndarray,
    ...
    for partial_permutation in permutations(points):
        ...
        distance = _compute_permutation_distance(distance_matrix, permutation)
