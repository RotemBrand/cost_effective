...
def solve_min_2edge(
    ...
    *,
    ...
    while cut_round < max_cut_rounds:
        ...
        solution.add_edges_from(selected_edges)
        ...
        total_added_cuts += cuts_added
        model.update()
        if cuts_added == 0:
            stop_reason = "no_violated_cuts_found"
            break
...
def write_solution(
    ...
    if OUTPUT_GPKG.exists():
        try:
            ...
            output_gpkg = OUTPUT_GPKG.with_name(f"{OUTPUT_GPKG.stem}_{int(time.time())}{OUTPUT_GPKG.suffix}")
